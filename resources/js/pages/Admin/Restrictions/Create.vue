<script setup lang="ts">
    ...
    // Minimum date for start_date (today)
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // Minimum date for end_date (start_date)
    const minEndDate = computed(() => {
        return form.start_date ? new Date(form.start_date) : today;
    });
</script>

<template>
    <Head title="Create Restriction" />

    <AppLayout>
        <!-- Page Header -->
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Create Restriction</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Add a new unavailable period or time block</p>
            </div>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription> Define when you are unavailable or when the resource is blocked </CardDescription>
            </CardHeader>
            <CardContent>
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex items-center gap-4 pt-4">
                        <Button type="submit" :disabled="form.processing">
                            ...
                            {{ form.processing ? 'Creating...' : 'Create Restriction' }}
                        </Button>
